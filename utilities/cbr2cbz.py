...
def cbr2cbz(
        ...
        , shrinkHeight=1500, whatif=False, mci=False, remove_sub_folder=True
):
        ...
        for root, dirs, files in os.walk(cbr2cbztemp):
            ...
            for leaf in files:
                ...
                try:
                    ...
                    imgar = imgx / imgy
                    if verbose > 4:
                        print("***** Imagestats:", imgsize, imgext, imgtype
                              , imgx, imgy, imgar)
                except:
                    if verbose > 3:
                ...
                if imgsize > shrinklimit or shrinkGray:
                    ...
                    try:
                        ...
                        if verbose > 4:
                            print(format(sys.exc_info()[0]))
                            print("ERROR: Could not convert file {0}"
                                  .format(shrinkfile))
                        if os.path.exists(shrinkfile + ".shrink.jpg"):
                            try:
                                os.unlink(shrinkfile + ".shrink.jpg")
    ...
    if remove_sub_folder:
        found_sub_folders = []
        for zf in zipfiles:
            # check for multiple sub-directories
            if '/' in zf:
                if zf.split('/')[0] not in found_sub_folders:
                    found_sub_folders.append(zf.split('/')[0])
        if len(found_sub_folders) == 0:
            if verbose > 2:
                print('*** Did not file sub-directories.')
        elif len(found_sub_folders) > 1:
            if verbose > 2:
                print('*** Found more than one sub-directory.  Will not try to remove sub-directories to root.')
        else:
            if verbose > 2:
                print(f'*** Moving all files from sub-directory to root')
            for idx, zf in enumerate(zipfiles):
                if len(zf.split('/')) == 2:  # make sure we are only 1 directory deep
                    os.rename(f'{cbr2cbztemp}/{zf}', f'{cbr2cbztemp}/{zf.split("/")[1]}')
                    zipfiles[idx] = f'{cbr2cbztemp.replace(cbr2cbztemp, "")}{zf.split("/")[1]}'

    if mci and not remove_sub_folder:
        already_moved = False
        found_xml_in_root = False
        found_sub_folders = []
        for zf in zipfiles:
            # check for multiple sub-directories
            if '/' in zf:
                if zf.split('/')[0] not in found_sub_folders:
                    found_sub_folders.append(zf.split('/')[0])
            # check if ComicInfo.xml in root
            if 'ComicInfo.xml' in zf and '/' not in zf:
                found_xml_in_root = True
        if found_xml_in_root:
            if verbose > 2:
                print('*** Found ComicInfo.xml in root.  Will not try to move any further found ComicInfo.xml files.')
        elif len(found_sub_folders) > 1:
            if verbose > 2:
                print('*** Found more than one sub-directory.  Will not try to move ComicInfo.xml.')
        else:
            for idx, zf in enumerate(zipfiles):
                if not already_moved:
                    if 'ComicInfo.xml' in zf and '/' in zf:
                        if len(zf.split('/')) == 2:  # make sure we are only 1 directory deep
                            if verbose > 2:
                                print(f'*** Found ComicInfo.xml in sub-directory.  Will move to root.')
                            os.rename(f'{cbr2cbztemp}/{zf}', f'{cbr2cbztemp}/ComicInfo.xml')
                            zipfiles[idx] = f'{cbr2cbztemp.replace(cbr2cbztemp, "")}ComicInfo.xml'
                            already_moved = True
                        else:
                            print(f'*** FOUND ComicInfo.xml in sub-directory of sub-directory.  Will not move.')
...
"""
    ...
    parser.add_argument(
        "--mci", "--move-comicinfo", default=False, action="store_true", dest="mci"
        , help="Move ComicInfo.xml from sub-folder to root")
    parser.add_argument(
        "--remove_sub_folder", default=False, action="store_true", dest="remove_sub_folder"
        , help="Move ComicInfo.xml from sub-folder to root")
            ...
            if convertflag:
                if options.whatif:
                    ...
                    if (cbr2cbz(
                            ...
                            , mci=options.mci
                            , remove_sub_folder=options.remove_sub_folder
                    )
                    ):
